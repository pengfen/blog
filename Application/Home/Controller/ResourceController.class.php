...
class ResourceController extends HomeController{
	...
	public function index(){
		...
		// if(!$data){
			$resource=D('Resource');
			$sql="select r.id,r.type,r.header,r.main,r.score,r.download,r.addtime,u.username from blog_resource r left join blog_user u on r.uid=u.id order by r.download desc";
	...
	// 	);
		...
		// if(!$datas){
			$resource=D('Resource');
			$sql="select r.id,r.type,r.header,r.main,r.score,r.download,r.addtime,u.username from blog_resource r left join blog_user u on r.uid=u.id where r.type='文学'";
	...
	public function technology(){
		$resource=D('Resource');	
		$sql="select r.id,r.type,r.header,r.main,r.score,r.download,r.addtime,u.username from blog_resource r left join blog_user u on r.uid=u.id where r.type='技术'";
		$data=$resource->query($sql);
		$this->assign('data',$data);
		$this->display();
	}
	...
	public function education(){
		$resource=D('Resource');
		$sql="select r.id,r.type,r.header,r.main,r.score,r.download,r.addtime,u.username from blog_resource r left join blog_user u on r.uid=u.id where r.type='教育'";
		$data=$resource->query($sql);
		$this->assign('data',$data);
		$this->display();
	}
